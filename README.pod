...
L<02packages.details.txt|https://cpan.metacpan.org/modules/02packages.details.txt>.
...
file L<06perms.txt|https://cpan.metacpan.org/modules/06perms.txt>. This is the
...
C<first-come> permission has ultimate authority on the name, and can add or
remove any other user's permissions. The C<permission manager> permission can
add or remove other user's permissions other than C<first-come>. The
C<co-maint> permission can only remove their own permission. Note that these
module permissions are B<not> hierarchical. An author must have permissions for
each individual module name they wish to have indexed.

The C<Author Search> searches the CPAN authors database, which is the file
L<00whois.xml|https://cpan.metacpan.org/authors/00whois.xml>. This contains
