...
installation path, such as C<https://cpanmeta.grinnz.com/>. All endpoints
...
C<other_authors> option is enabled, details are returned for all authors with
permissions for the matched modules, not just the specified author CPAN ID.
...
CPAN IDs starting with the specified prefix.
