...
for CPAN (Comprehensive Perl Archive Network). The first part is a C<refresh>
command which will download this metadata and index it into the selected
backend database. The second part is a web application which provides search
functionality into the indexed database.
...
Perl dependencies are listed in F<cpanfile>. They can be installed by running:
...
The C<redis> backend requires L<Mojo::Redis2> and a Redis server running at least
